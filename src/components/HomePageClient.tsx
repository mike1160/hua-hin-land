...
import { truncateText, type PublicListing } from '@/lib/listings'
...
export default function HomePageClient({ previewListings }: Props) {
    ...
    <main className="bg-[#FAF7F0] text-[#1A2744] font-sans overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex flex-col">
        ...
        <div className="relative z-10 max-w-5xl mx-auto w-full px-6 pt-[35vh] pb-24">
          <div className="inline-flex items-center rounded-full px-4 py-1.5 text-xs text-white/90 mb-3 font-medium tracking-wide bg-white/20 border border-white/30 backdrop-blur-sm">
            {t('marketplaceBadge')}
          </div>
      ...
      <section className="bg-[#FAF7F0] border-b border-[#E8E2D6]">
        <div className="max-w-5xl mx-auto px-6 py-10 grid sm:grid-cols-3 gap-8 text-center">
          {[
            { valueKey: 'statsFeaturedValue', labelKey: 'statsFeaturedLabel' },
            { valueKey: 'statsChanoteValue', labelKey: 'statsChanoteLabel' },
            { valueKey: 'statsBeachValue', labelKey: 'statsBeachLabel' },
          ].map((item) => (
            <div key={item.valueKey}>
              <p className="text-2xl md:text-3xl font-bold text-amber-600" style={{ fontFamily: 'Playfair Display, serif' }}>
                {t(item.valueKey)}
              </p>
              <p className="text-[#5C5247] text-sm mt-1">{t(item.labelKey)}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* LISTINGS PREVIEW */}
      <section className="bg-white border-b border-[#E8E2D6] py-16 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            <h2
              className="text-3xl md:text-4xl font-bold text-[#1A2744] mb-3"
              style={{ fontFamily: 'Playfair Display, serif' }}
            >
              {t('listingsPreviewTitle')}
            </h2>
            <p className="text-[#5C5247] text-sm md:text-base">{t('listingsPreviewSubtitle')}</p>
          </div>
          ...
          {previewListings.length === 0 ? (
            ...
            <div className="grid gap-5 md:grid-cols-3">
              {previewListings.map((listing) => (
                <article
                  key={listing.id}
                  className="bg-[#FAF7F0] border border-[#E8E2D6] rounded-[12px] p-5 md:p-6 flex flex-col"
                >
                  ...
                  <h3
                    className="text-lg font-bold text-[#1A2744] mb-2"
                    style={{ fontFamily: 'Playfair Display, serif' }}
                  >
                  ...
                  <dl className="space-y-1.5 text-sm mb-3">
                    {listing.size ? (
                      <div className="flex justify-between gap-3">
                        <dt className="text-[#5C5247]">{tl('size')}</dt>
                        <dd className="text-[#1A2744] font-medium text-right">{listing.size}</dd>
                      </div>
                    ) : null}
                    {listing.title_deed ? (
                      <div className="flex justify-between gap-3">
                        <dt className="text-[#5C5247]">{tl('titleDeed')}</dt>
                        <dd className="text-[#1A2744] font-medium text-right">{listing.title_deed}</dd>
                      </div>
                    ) : null}
                  </dl>
                  {listing.description ? (
                    <p className="text-[#5C5247] text-sm leading-relaxed mb-5 flex-1">
      ...
      {/* PHOTO GALLERY — featured own plot */}
      <section className="relative py-10 px-6 bg-[#FAF7F0]">
        <div className="max-w-5xl mx-auto">
          <div className="inline-flex items-center gap-1.5 mb-4 px-3 py-1.5 rounded-md text-xs font-semibold bg-amber-600 text-white">
            {t('featuredBadge')}
          </div>
      ...
      {/* LOCAL AGENT */}
      <section className="py-16 px-6 bg-white border-y border-[#E8E2D6]">
        <div className="max-w-5xl mx-auto">
          <h2
            className="text-3xl md:text-4xl font-bold text-[#1A2744] mb-8 text-center"
            style={{ fontFamily: 'Playfair Display, serif' }}
          >
            {t('agentTitle')}
          </h2>
          <div className="max-w-md mx-auto bg-[#FAF7F0] border border-amber-600/40 rounded-[12px] p-6 md:p-8 flex items-start gap-4">
            <div
              className="flex-shrink-0 w-14 h-14 rounded-full bg-[#1A2744]/10 border border-amber-600/30 flex items-center justify-center text-[#1A2744]"
              aria-hidden
            >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[#1A2744] font-semibold text-lg">{t('agentName')}</p>
              <p className="text-[#5C5247] text-sm mt-0.5">{t('agentRole')}</p>
              <a
                href={`tel:${PHONE}`}
                className="inline-block text-[#1A2744] font-medium text-sm mt-3 hover:text-amber-700 transition-colors"
              >
                {PHONE_DISPLAY}
              </a>
              <div className="mt-4">
                <a
                  href="https://wa.me/66659012984"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center justify-center min-h-[40px] px-4 rounded-lg text-sm font-semibold border border-amber-600 text-amber-700 hover:bg-amber-600 hover:text-white transition-colors"
                >
                  {t('agentWhatsApp')}
                </a>
              </div>
