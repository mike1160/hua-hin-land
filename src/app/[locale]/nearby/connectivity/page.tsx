    ...
    <main className="min-h-screen bg-[#FAF7F0] text-[#1A2744]">
      <div className="relative h-72 flex items-end pb-10 px-6">
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <h1 className="text-white text-4xl md:text-5xl font-bold mt-2" style={{fontFamily: 'Playfair Display, serif', textShadow: '0 2px 8px rgba(0,0,0,0.5)'}}>Getting around — better connected than you think</h1>
          <p className="text-white/85 mt-2">Routes, airports &amp; future rail links from Soi 112</p>
        </div>
      </div>
      <div className="max-w-3xl mx-auto px-6 py-12 space-y-8">
        ...
        <figure>
          ...
          <figcaption className="text-[#5C5247] text-xs mt-2">Plot location · Soi 112, Thap Tai · {PLOT_LAT}, {PLOT_LNG}</figcaption>
        </figure>

        <div className="space-y-4">
          <h2 className="text-[#1A2744] text-xl font-semibold">🛣️ Three routes from the plot</h2>
          {routes.map((route) => (
            <div key={route.title} className="bg-white border border-[#E8E2D6] rounded-[12px] p-6">
              <div className="flex items-start justify-between gap-4 mb-2">
                <div className="flex items-center gap-3">
                  <span className="text-2xl">{route.icon}</span>
                  <div>
                    <p className="text-[#1A2744] font-semibold text-sm">{route.title}</p>
                    <p className="text-[#5C5247] text-xs">{route.destination}</p>
                  </div>
                </div>
                <span className="text-[#C8973A] font-bold text-sm flex-shrink-0">{route.time}</span>
              </div>
              <p className="text-[#5C5247] text-sm leading-relaxed pl-10">{route.detail}</p>
            </div>
          ))}
        </div>
        ...
        <figure>
          ...
          />
          <figcaption className="text-[#5C5247] text-xs mt-2">Aerial view · Soi 112 corridor · plot centre</figcaption>
        </figure>

        <div className="bg-white border border-[#E8E2D6] rounded-[12px] p-6">
          <h2 className="text-[#1A2744] text-xl font-semibold mb-4">✈️ Distances &amp; travel times</h2>
          <div className="space-y-3 text-sm">
            <div className="flex justify-between border-b border-[#E8E2D6] pb-2"><span className="text-[#5C5247]">Hua Hin town centre</span><span className="text-[#C8973A] font-medium">15 min</span></div>
            <div className="flex justify-between border-b border-[#E8E2D6] pb-2"><span className="text-[#5C5247]">Hua Hin Airport</span><span className="text-[#C8973A] font-medium">25 min</span></div>
            <div className="flex justify-between border-b border-[#E8E2D6] pb-2"><span className="text-[#5C5247]">Pranburi coast</span><span className="text-[#C8973A] font-medium">20 min</span></div>
            <div className="flex justify-between border-b border-[#E8E2D6] pb-2"><span className="text-[#5C5247]">Bangkok (Suvarnabhumi)</span><span className="text-[#C8973A] font-medium">3.5 hrs by car</span></div>
            <div className="flex justify-between"><span className="text-[#5C5247]">Bangkok-Hua Hin high speed rail</span><span className="text-[#C8973A] font-medium">Upcoming — significantly faster</span></div>
          </div>
        </div>

        <div className="bg-white border border-[#E8E2D6] rounded-[12px] p-6">
          <h2 className="text-[#1A2744] text-xl font-semibold mb-4">🚄 Future connectivity</h2>
          <p className="text-[#5C5247] text-sm leading-relaxed">Upcoming Bangkok-Hua Hin high speed rail will reduce Bangkok travel time significantly once operational. Soi 112&apos;s position on Route 2004 means the plot benefits from improved regional connectivity without being on a busy through-road.</p>
        </div>
