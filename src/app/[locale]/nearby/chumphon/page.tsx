...
export default function Chumphon() {
  const t = useTranslations('chumphonPage')
    ...
    <main className="min-h-screen bg-[#FAF7F0] text-[#1A2744]">
      <div className="relative h-72 flex items-end pb-10 px-6">
        <div className="absolute inset-0">
          <img src="/photo2.jpg" alt={t('heroAlt')} className="w-full h-full object-cover" style={{objectPosition: 'center 60%'}} />
      ...
      <div className="max-w-3xl mx-auto px-6 py-12 space-y-8">
        <div className="bg-white border border-[#E8E2D6] rounded-[12px] p-6">
          ...
          <p className="text-[#5C5247] text-sm leading-relaxed mb-4">{t('whyBody')}</p>
        ...
        <div className="bg-white border border-[#E8E2D6] rounded-[12px] p-6">
          <h2 className="text-[#1A2744] text-xl font-semibold mb-4">{t('biggerTitle')}</h2>
          <p className="text-[#5C5247] text-sm leading-relaxed">{t('biggerBody1')}</p>
          <p className="text-[#5C5247] text-sm leading-relaxed mt-3">{t('biggerBody2')}</p>
        </div>
      </div>
