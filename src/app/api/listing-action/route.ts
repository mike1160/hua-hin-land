  ...
  try {
    ...
    if (listing.email) {
      ...
      await resend.emails.send({
        ...
        to: listing.email,
        subject: isApprove
          ? 'Your listing has been accepted — Hua Hin Land'
          : 'Update on your listing submission — Hua Hin Land',
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0d1120; color: #f0f4ff; padding: 32px; border-radius: 12px;">
            <h2 style="color: ${isApprove ? '#16a34a' : '#f59e0b'}; margin-bottom: 16px;">
              ${isApprove ? 'Listing accepted' : 'Listing not published'}
            </h2>
            <p style="color: #cbd5e1; line-height: 1.7; margin: 0;">
              ${
                isApprove
                  ? 'Your listing has been accepted and will appear on hua-hin-land.com shortly.'
                  : 'Thank you for your submission. Unfortunately your listing did not meet our criteria.'
              }
            </p>
            <p style="margin: 24px 0 0; color: #64748b; font-size: 12px;">Hua Hin Land · hua-hin-land.com</p>
          </div>
        `,
      })
    }
